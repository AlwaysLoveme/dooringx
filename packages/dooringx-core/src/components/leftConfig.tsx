...
import { dragEventResolve, LeftRegisterComponentMapItem } from '@/core/crossDrag';
...
function LeftConfig(props: LeftConfigProps) {
	...
	useEffect(() => {
		let cache: LeftRegisterComponentMapItem[] = [];
			...
			setLeftRender(
				<div className={`${styles.leftco} yh-leftcomp`}>
					...
					{search &&
						search !== '' &&
						cache
							.reduce<LeftRegisterComponentMapItem[]>((prev, next) => {
	...
	const [renderCollapse, setRenderCollapse] = useState(props.config.getCollapse());
