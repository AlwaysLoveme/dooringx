...
import ModalMask from '@/core/components/defaultFormComponents/modalMask';
...
export interface InitConfig {
	 ...
	 * @memberof InitConfig
	 */
	leftAllRegistMap: LeftRegisterComponentMapItem[];
	/**
	 * 左边tab页图标配置
...
export const defaultConfig: InitConfig = {
	...
	initComponentCache: {
		modalMask: { component: ModalMask }, // 这个组件不配置显示
	},
	initFunctionMap: {
		open_modal: {
...
export class UserConfig {
	 ...
	 * @memberof UserConfig
	 */
	addCoRegistMap(v: LeftRegisterComponentMapItem) {
		const obj = {} as InitConfig;
	...
	scriptRegistComponentSingle(item: ComponentItemFactory, leftProps: LeftRegisterComponentMapItem) {
	...
	// 	leftProps: LeftRegisterComponentMapItem[]
	// ) {
	// 	items.forEach((item) => {
	// 		this.registComponent(item);
	// 	});
	 ...
	 * @param {Partial<LeftRegisterComponentMapItem>} leftProps 分类
	...
	scriptSingleLoad(
		src: string,
		leftProps: Partial<LeftRegisterComponentMapItem>,
		callback?: Function
	) {
		...
		if (!this.scriptLoading) {
			...
			script.onload = () => {
				...
				try {
					...
					this.scriptRegistComponentSingle(item, left as LeftRegisterComponentMapItem);
				} catch (e) {
					console.error(e);
				}
